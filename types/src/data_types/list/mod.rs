...
impl List {
    ...
    // Todo: Is it neccessary to check if list contains todos?
...
mod tests_mod {
    ...
    use crate::{List, SourceFile, Todo};
    use std::collections::HashMap;
    ...
    fn get_todos_grouped_by_files() {
        ...
        let ordered_list = list.group_by_file();
    ...
    fn try_to_group_todos_of_empty_list() {
        let list = List::new("My List".to_string(), vec![]);
        let grouped = list.group_by_file();
